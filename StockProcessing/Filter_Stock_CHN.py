...
def calBasic():
    ...
    grow_symbols = []
      ...
      if os.path.exists("year_2019.csv") == False:
        df_main = ts.get_report_data(2019, 4)
        mainData = pd.DataFrame(df_main)
        mainData.to_csv("year_2019.csv")
        
  if os.path.exists("year_2018.csv") == False:
        df_main = ts.get_report_data(2018, 4)
        mainData = pd.DataFrame(df_main)
        mainData.to_csv("year_2018.csv")
        
        
  if os.path.exists("year_2017.csv") == False:
        df_main = ts.get_report_data(2017, 4)
        mainData = pd.DataFrame(df_main)
        mainData.to_csv("year_2017.csv")
